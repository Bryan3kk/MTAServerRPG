--[Sistema de Combustivel "L1K3"]--
marker1 = createMarker(1943.6787109375,-1778.5,12.390598297119,"cylinder",2,150,255,150,150)
blip1 = createBlip(1943.6787109375,-1778.5,12.390598297119,44,4,0,0,0,0,0,200)
...
MaxFuel = 100 -- 100 padrão de máximo de combustivel.
decreasing = 0.0010 -- per frame.
increasing = 0.35 -- per frame. (quadno abastecendo)
price = 3 -- $3 é padrao. When changing this variable, make sure that the (new price * increasing speed) > 1, else it won't take player money. 
...
function getElementSpeed(element,unit)
	...
	if (isElement(element)) then
		...
		outputDebugString("Não é um elemento, não é possivel coletar a velocidade")
		return false
	end
end
...
for f,v in ipairs(getElementsByType("vehicle"))do
	local fuel = getElementData(v,"fuel")
		if getVehicleEngineState(v) == true and (not getElementData(v,"fuel") == false) then
			setElementData(v,"fuel",fuel - getElementSpeed(v,"kph")*0.00005 - decreasing)
		end
	end
end
...
dxDrawImage (x*0.62, y*0.78,226, 223,"fuelcircle.png",0,0,0)
dxDrawImage (x*0.62, y*0.8,226, 223,"fuelarrow.png",getElementData(veh,"fuel") + 230,0,0)
dxDrawImage (x*0.77,y*0.63,300,300,"speedcircle.png")
dxDrawImage (x*0.77,y*0.63,300,300,"speedarrow.png",getElementSpeed(veh,"mph")*0.74 - 11,0,0)
dxDrawImage (x*0.87,y*0.788,50,50,"carhealth.png",getElementRotation(veh),0,0,tocolor(0 - getElementHealth(veh)*0.255,getElementHealth(veh)*0.255,0,255))
end
end
...
outputChatBox("Desligue o motor do Carro para abastecer",0,255,0)
    function showDx()
        dxDrawRectangle(x*0.452, y*0.290, x*0.396, y*0.2, tocolor(1, 1, 1, 112), false)
        dxDrawRectangle(x*0.452, y*0.290, x*0.396, y*0.25, tocolor(1, 1, 1, 112), false)
        dxDrawText("Posto de Combustível", x*0.546, y*0.295, x*0.760, y*0.325, tocolor(255, 255, 255, 254), 1.00, "bankgothic", "center", "center", false, false, true, false, false)
        dxDrawText("Combustível Atual : "..math.floor(getElementData(veh,"fuel")*1000)/1000, x*0.457, y*0.332, x*0.842, y*0.371, tocolor(255, 255, 255, 255), 1.10, "sans", "left", "center", false, false, true, false, false)
        if (MaxFuel - getElementData(veh,"fuel")) < 1 then
		dxDrawText("Combustível Necessário : "..(MaxFuel) - math.floor(getElementData(veh,"fuel")), x*0.457, y*0.371, x*0.842, y*0.410, tocolor(255, 255, 255, 255), 1.10, "sans", "left", "center", false, false, true, false, false)
        else
		dxDrawText("Combustível Necessário : "..(MaxFuel) - (math.floor(getElementData(veh,"fuel")*1000)/1000), x*0.457, y*0.371, x*0.842, y*0.410, tocolor(255, 255, 255, 255), 1.10, "sans", "left", "center", false, false, true, false, false)
		end
		dxDrawText("Preço do Litro : $"..price, x*0.457, y*0.410, x*0.842, y*0.449, tocolor(255, 255, 255, 255), 1.10, "sans", "left", "center", false, false, true, false, false)
       dxDrawText("Custo total : "..math.floor(getElementData(localPlayer,"price")*100)/100, x*0.457, y*0.449, x*0.842, y*0.488, tocolor(255, 255, 255, 255), 1.10, "sans", "left", "center", false, false, true, false, false)
        if getVehicleEngineState(veh) == true then
		dxDrawText("Desligue o motor para abastecer", x*0.457, y*0.488, x*0.842, y*0.527, tocolor(255, 255, 255, 255), 1.10, "sans", "center", "center", false, false, true, false, false)
		else
		dxDrawText("Ligue o motor", x*0.457, y*0.488, x*0.842, y*0.527, tocolor(255, 255, 255, 255), 1.10, "sans", "center", "center", false, false, true, false, false)
		end
	end
...
setElementData(hitElement,"refuelling",true) -- Téc Dyo (01/07/2017)
setElementData(hitElement,"price",0)
end
...
veh = getPedOccupiedVehicle(localPlayer)
	if veh and (not getElementData(veh,"fuel") == false ) and getElementData(veh,"fuel") <= 0 then
		...
		setVehicleEngineState(veh,true)-- Alterado (Téc. Dyovani)
	end
end
