  ...
  if isObjectInACLGroup( 'user.'..getAccountName( getPlayerAccount( player ) ), aclGetGroup( 'Admin' ) ) or hasObjectPermissionTo( player, 'function.banPlayer', false ) then
    if getElementData( player, 'HP_Opened' ) then
      ...
      if r and r > 0 and r <= #ints then
        ...
        outputChatBox( '* ID interior incorreta!', player, 255, 51, 36 );
      end;
    else
      outputChatBox( '* Acesso negado para este comando: Casa do painel de construção deve ser aberta!', player, 255, 51, 36 );
    end;
  else
    outputChatBox( '* Acesso negado para este comando!', player, 255, 51, 36 );
  end;
end );